...
class GameCoreController:
    ...
    def __init__(self):
        ...
        # ]
        # 用于存储去零和合并的列表
        self.__list_merge = []#用于存储去零合并的列表
        # 用于存储空位置的列表
        self.__list_empty_location = []#用于存储空位置的列表
        # 地图是否发生变化
        self.is_change = False#给is_change赋初始值
    ...
    def is_change(self, value):
        self.__is_change = value#为什么这里会有个value？

    def __zero_to_end(self):#把零移到最后
        for i in range(len(self.__list_merge) - 1, -1, -1):
            if self.__list_merge[i] == 0:
                del self.__list_merge[i]
                self.__list_merge.append(0)

    def __merge(self):#相同的数相加
        self.__zero_to_end()
    ...
    def __move_left(self):
        for r in range(len(self.__map)):#为什么要用切片？
